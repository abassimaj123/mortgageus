...
void main() {
  ...
  group('calcARM — balance at reset', () {
    ...
    test('ARM3: balance at reset is less than original loan (amortisation happened)', () {
      final arm = MortgageCalculator.calcARM(
        loanAmount:      300000,
        initialRatePct:  6.5,
        fixedYears:      5,
      ...
      expect(arm.balanceAtReset, greaterThan(250000)); // 5yrs at 6.5%: rough range
    });
  ...
  group('calcARM — interest direction', () {
    ...
    test('ARM5: adjusted rate > initial → ARM total interest > fixed total interest', () {
      final arm = MortgageCalculator.calcARM(
        loanAmount:      300000,
        initialRatePct:  6.5,
        fixedYears:      5,
    ...
    test('ARM6: adjusted rate < initial → ARM total interest < fixed total interest', () {
      final arm = MortgageCalculator.calcARM(
        loanAmount:      300000,
        initialRatePct:  6.5,
        fixedYears:      5,
  ...
  group('calcARM — break-even logic', () {
    ...
    test('ARM9: adjusted rate < initial → breakEvenMonths is null (ARM always cheaper)', () {
      final arm = MortgageCalculator.calcARM(
        loanAmount:      300000,
        initialRatePct:  6.5,
        fixedYears:      5,
  ...
  group('calcARM — total cost integrity', () {
    ...
    test('ARM12: totalCost = balanceAtReset + remaining balance + all interest (approximate)', () {
      final arm = MortgageCalculator.calcARM(
        loanAmount:      300000,
        initialRatePct:  6.5,
        fixedYears:      5,
    ...
    test('ARM13: 10yr fixed period has higher balance at reset than 3yr (less paid down)', () {
      final arm5 = MortgageCalculator.calcARM(
        loanAmount: 300000, initialRatePct: 6.5,
        fixedYears: 5, adjustedRatePct: 7.0, totalTermYears: 30,
      );
      final arm10 = MortgageCalculator.calcARM(
        loanAmount: 300000, initialRatePct: 6.5,
        fixedYears: 10, adjustedRatePct: 7.0, totalTermYears: 30,
      );
  ...
  group('calcARM — argument validation', () {
    ...
    test('ARM14: loan = 0 throws ArgumentError', () {
      expect(
        () => MortgageCalculator.calcARM(
          loanAmount: 0, initialRatePct: 6.5,
          fixedYears: 5, adjustedRatePct: 7.0, totalTermYears: 30),
        throwsArgumentError,
      );
    });
    ...
    test('ARM15: fixedYears >= totalTermYears throws ArgumentError', () {
      expect(
        () => MortgageCalculator.calcARM(
          loanAmount: 300000, initialRatePct: 6.5,
          fixedYears: 30, adjustedRatePct: 7.0, totalTermYears: 30),
        throwsArgumentError,
      );
    });
    ...
    test('ARM16: negative initialRate throws ArgumentError', () {
      expect(
        () => MortgageCalculator.calcARM(
          loanAmount: 300000, initialRatePct: -1.0,
          fixedYears: 5, adjustedRatePct: 7.0, totalTermYears: 30),
        throwsArgumentError,
      );
    });
