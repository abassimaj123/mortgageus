...
class MortgageInputNotifier extends StateNotifier<MortgageInputState> {
  ...
  void updateDownPaymentPct(double v) =>
      state = state.copyWith(downPaymentPct: v);
  ...
  void updatePropertyTaxRate(double v) =>
      state = state.copyWith(propertyTaxRatePct: v);
  void updateHomeInsurance(double v) =>
      state = state.copyWith(homeInsuranceAnnual: v);
  void updateHoa(double v) => state = state.copyWith(hoaMonthly: v);
  void toggleDownPaymentMode(bool dollar) =>
      state = state.copyWith(downPaymentAsDollar: dollar);
}
...
final mortgageResultProvider = Provider<MortgageResult?>((ref) {
  ...
  if (input.homePrice <= 0 || input.termYears <= 0 || input.annualRatePct < 0)
    return null;
